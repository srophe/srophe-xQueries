...
for $person-record in $persons[descendant-or-self::*[contains(@xml:id,'bib-')]]
    ...
    let $update-node := 
        for $node in $person/descendant-or-self::*[contains(@xml:id,'bib-')]
(:        let $prefix := :)
...
(:                    1) :)
(:            else():)
        let $xml-id := replace($node/@xml:id,'bib\-',concat('bib',$person-id,'-'))
