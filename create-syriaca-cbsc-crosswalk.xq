...
(:
: Creates a CSV file that crosswalks between Syriaca URIs and English Headwords
: and the CBSC keyword and browse link based on what data exists in the Syriaca
: entity record. Checks in persons, places, and works. Last run 2021-12-02.
:
...
let $persons := 
  ...
  let $hasCbscData := boolean(($cbscIdno || $cbscBiblTarget || $cbscBiblKeyword) != "")
  return if($hasCbscData) then
  <record>
    <syriacaUri>{$recordUri}</syriacaUri>
    <syriacaEnglishHeadword>{$enHeadword}</syriacaEnglishHeadword>
    <category>person</category>
    <cbscIdnoLink>{$cbscIdno}</cbscIdnoLink>
    <cbscBiblLink>{$cbscBiblTarget}</cbscBiblLink>
    <cbscBiblKeyword>{$cbscBiblKeyword}</cbscBiblKeyword>
  </record>
