...
(: config declarations :)
let $csvUri := "C:\Users\anoni\Documents\GitHub\miscellaneous-util-scripts\Syriac-World-Maps-Util\RoutledgeSyriacWorldMaps2016-ListofPlaces_November17RevisedPlaceNames.csv" (: specify sheet :)
...
let $compareCells := "New_ID" (:the header name of the cell you wish to compare:)
let $returnCells := "My_notes_from_Nov17Revised" (: specify the header names for the desired data cells; may be a sequence -- can't seem to get that to work without a lot of headache. :)
...
for $rec in $doc/*:list/*:record
  ...
  return insert node $newData into $rec
