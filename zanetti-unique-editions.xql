...
let $new-doc := doc("/db/apps/srophe-data/data/bibl/zanetti-editions.xml")/TEI
(: Run on all works :)
let $work := collection('/db/apps/srophe-data/data/works/tei')//body/bibl
let $editions := 
    for $edition in $work/note[@type='editions' or @type='ancientVersion' or @type='modernTranslation']
    return <bibl>{$edition}</bibl>
(:    return functx:remove-attributes(functx:remove-elements($edition,'citedRange'),'xml:id'):)
...
(:    $unique-editions-w-id:)
    update insert $unique-editions-w-id into $new-doc