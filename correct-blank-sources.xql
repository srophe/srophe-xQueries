...
for $person-record in $persons[descendant-or-self::*[contains(@source,'#bib-')]]
    ...
    let $update-node := 
        for $node in $person/descendant-or-self::*[contains(@source,'#bib-')]
(:        let $prefix := :)
...
(:                    1) :)
(:            else():)
        let $new-source := replace($node/@source,'#bib\-',concat('#bib',$person-id,'-'))
        return update replace $node with syriaca:update-attribute($node, 'source', $new-source)
    return $update-node